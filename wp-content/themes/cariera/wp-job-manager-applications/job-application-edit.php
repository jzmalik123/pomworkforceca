...
<form class="job-manager-application-edit-form job-manager-form" method="post">
	...
	<fieldset class="fieldset-rating">
		<label for="application-rating-<?php absint( $application->ID ); ?>"><?php esc_html_e( 'Rating (out of 5)', 'cariera' ); ?>:</label>
		<div class="field">
			<input type="number" id="application-rating-<?php echo absint( $application->ID ); ?>" name="application_rating" step="0.5" max="5" min="0" placeholder="0" value="<?php echo esc_attr( get_job_application_rating( $application->ID ) ); ?>" />
		</div>
	</fieldset>
